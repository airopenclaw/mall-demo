...
(function () {
    ...
    function initializeMainFrame() {
        ...
        // 注入样式：隐藏 Axure mockup 底图
        $(mainFrame).on('load', function() {
            try {
                var doc = mainFrame.contentDocument || mainFrame.contentWindow.document;
                if (doc && doc.readyState === 'complete') {
                    injectMockupStyle(doc);
                } else {
                    $(mainFrame.contentWindow).on('load', function() {
                        injectMockupStyle(mainFrame.contentDocument || mainFrame.contentWindow.document);
                    });
                }
            } catch(e) {
                // cross-origin, skip
    ...
    function injectMockupStyle(doc) {
        if (!doc || doc.getElementById('mockup-inject-style')) return;
        try {
            var style = doc.createElement('style');
            style.id = 'mockup-inject-style';
            style.textContent = 'img[id^="u"][id$="_img"]{display:none!important;}body,#base,html{background-color:#ffffff!important;background-image:none!important;}';
            doc.head.appendChild(style);
            var base = doc.getElementById('base');
            if (base) { base.style.backgroundImage = 'none'; base.style.backgroundColor = '#ffffff'; }
            if (doc.body) { doc.body.style.backgroundColor = '#ffffff'; doc.body.style.backgroundImage = 'none'; }
        } catch(e) {}
    }
